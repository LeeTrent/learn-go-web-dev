...
func logout(resp http.ResponseWriter, req *http.Request) {
	if alreadyLoggedIn(resp, req) {
		http.Redirect(resp, req, "/", http.StatusSeeOther)
		return
	}

	// We need the cookie value in order to obtain the Session ID so we can delete the HTTP Session
	cookie, err := cookieutil.GetCookie(cookieName, req)
	if err == nil {
		sessionId := cookie.Value
		sessionMgr.RemoveSession(sessionId)
	}
	// DeleteandSetCookie() creates a new cookie with a value of "" and a max age of -1
	cookieutil.DeleteAndSetCookie(cookieName, resp)
